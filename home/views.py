...
from .models import ayushVaccineSlot
        ...
        for each in response:  # run loop 10 times
            for session in each['sessions']:
                if session['available_capacity_dose1'] >= 0:
                    ...
                    all_data.save()
                    all_api_data = ayushVaccineSlot.objects.all().order_by('id')
    return all_api_data
...
def verification_view(request):
    if request.user.is_superuser:
        messages = ""
        if request.method == "POST":
            # vaccineBookingObj = vaccineBookingClass()
            # vaccineBookingObj.save()
            try:
                ...
                member_profile_data = Member_info_Class.objects.get(
                    uid=request.POST['uid'])    # change uid to id
